...
func NewRPCHeaderCodec(codec msgcodec.RPCHeader) RPCHeaderCodec {
	return RPCHeaderCodec{codec}
}

func (h *RPCHeaderCodec) Encode(header *msg.RPCHeader) []byte {
	return h.RPCHeader.Encode(header)
}

func (h *RPCHeaderCodec) Decode(conn net.Conn) (*msg.RPCHeader, error) {
	return h.RPCHeader.Decode(conn)
}
